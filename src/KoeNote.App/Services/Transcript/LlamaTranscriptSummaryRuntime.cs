...
public sealed class LlamaTranscriptSummaryRuntime(
    ...
    : ITranscriptSummaryRuntime, ITranscriptSummaryRuntimeSession
{
    ...
    public void EndSummarySession(TranscriptSummaryOptions options)
    {
        ...
        CancellationToken cancellationToken)
    {
        if (options.UseLlamaServerChatMtp)
        {
            return await RunServerChatMtpAsync(options, promptPath, cancellationToken).ConfigureAwait(false);
        }
    ...
    private async Task<ProcessRunResult> RunServerChatMtpAsync(
        TranscriptSummaryOptions options,
        string promptPath,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new ReviewWorkerException(
                    ReviewFailureCategory.ProcessFailed,
                    $"Transcript summary llama-server request failed with {(int)response.StatusCode}: {responseJson}");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var logDirectory = Path.Combine(options.OutputDirectory, "summary-llama-server-mtp");
            ...
            Process process;
            TranscriptSummaryOptions safeOptions;
            ...
            catch (Exception exception) when (LlamaRuntimePathBridge.IsBridgePreparationException(exception))
            {
                ...
                throw new ReviewWorkerException(
                    ReviewFailureCategory.ProcessFailed,
                    $"Could not prepare ASCII-safe summary llama-server runtime paths: {exception.Message}",
                    exception);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            if (process.HasExited)
            {
                throw new ReviewWorkerException(
                    ReviewFailureCategory.ProcessFailed,
                    $"summary llama-server exited before becoming healthy. exit_code={process.ExitCode}");
            }
        ...
        throw new TimeoutException($"summary llama-server did not become healthy within {timeout}.");
    }
    ...
    private static void ValidateServerInputs(TranscriptSummaryOptions options)
    {
    ...
    private static void ValidateServerCapability(TranscriptSummaryOptions options)
    {

...
public sealed record AsrSettings(string ContextText, string HotwordsText, string EngineId = VibeVoiceCrispAsrEngine.Id)
{
...
public sealed class AsrSettingsRepository(AppPaths paths)
{
    public AsrSettings Load()
    {
        ...
        command.CommandText = """
            SELECT context_text, hotwords_text, engine_id
            FROM asr_settings
            WHERE settings_id = 1;
            """;
        ...
        return new AsrSettings(reader.GetString(0), reader.GetString(1), reader.GetString(2));
    }
    ...
    public void Save(AsrSettings settings)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO asr_settings (settings_id, context_text, hotwords_text, engine_id, updated_at)
            VALUES (1, $context_text, $hotwords_text, $engine_id, $updated_at)

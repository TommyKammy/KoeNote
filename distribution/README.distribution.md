...
- Review GPU acceleration: NVIDIA CUDA DLLs are installed under `%LOCALAPPDATA%/KoeNote/runtimes/gpu/review-cuda/`; the CUDA bridge remains app-local at `tools/review/ggml-cuda.dll` so app updates refresh it with the matching `llama-completion.exe`.

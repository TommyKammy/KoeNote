...
- Review 用モデルは安定性を優先し、Gemma 4 E4B it Q4_K_M を推奨します。Gemma 4 12B it QAT Q4_0 は現時点では llama.cpp CUDA 経路で出力が不安定になる場合があるため、通常のモデル選択には表示しません。
...
4. 実行ボタンを押します。
5. 標準レイアウト中央の `整文` で、読みやすい文書としての結果を確認します。
6. 必要に応じて、`原文` に切り替えて ASR 原文を修正したり、右側の整文候補・要約を確認します。
...
KoeNote には、普段の確認に使いやすい `標準` レイアウトと、編集・比較・レビューをまとめて確認できる `詳細` レイアウトがあります。ヘッダー右側の `標準` / `詳細` でいつでも切り替えられ、選択中のジョブ、再生位置、タブ状態を保ったまま作業できます。

### 標準レイアウト

KoeNote は、日常的な確認作業の中心になる標準レイアウトで起動します。標準レイアウトは、左のジョブレール、中央の文書エリア、右の AI 支援レール、下部のスリム音声プレイヤーで構成されています。画面上部の `整文` / `原文` で文書エリアの表示を切り替え、必要なときだけ右側の AI 支援レールを開いて整文候補や要約を確認できます。

- 左のジョブレールでは、音声ファイルの追加、検索、状態確認、履歴整理を行います。
- 中央の文書エリアでは、`整文` と `原文` を切り替えて確認します。
- `整文` は読みやすい文書として整えた結果を表示します。フォント、余白、本文幅は文書確認に集中しやすい設定です。
- `原文` は ASR の素起こしを表示します。検索、話者フィルター、自動スクロールを使いながら、必要な箇所を修正できます。
- 右の AI 支援レールでは、整文候補の確認と要約の生成・再生成を行います。閉じている間も画面端の縦ラベルから状態を確認できます。
- 下部のスリム音声プレイヤーでは、再生・停止、前後セグメント移動、波形シーク、音量を操作できます。
...
- 標準 / 詳細: 普段の確認向けの標準レイアウトと、編集・比較向けの詳細レイアウトを切り替えます。
- 原文 / 整文: 標準レイアウト中央の表示対象を切り替えます。
- エクスポート: 現在表示している対象に合わせて、整文、素起こし、レビュー候補、要約を TXT / Markdown / JSON / SRT / VTT / DOCX / XLSX などの形式で出力します。タイムスタンプの有無や、同じ話者が連続するセグメントをまとめるかどうかも切り替えられます。
- ズーム: 文書エリアの表示倍率を調整します。
- VERSION / ASR モデル: 現在のバージョンと、文字起こしに使うモデルを表示します。
- 設定 / 更新確認: 右上のアイコンから、設定・辞書プリセット・モデル・セットアップ・ログや更新確認を開けます。

### 詳細レイアウト

詳細レイアウトは、確認・編集・比較・レビュー操作を広く並べた画面です。ヘッダーの `詳細` から切り替えられ、選択中のジョブ、再生位置、タブ状態を保ったまま開きます。
...
- 詳細レイアウトでは、従来相当のフル音声プレイヤーとレビュー操作パネルを維持しています。

### 整文候補・要約

標準レイアウト右側の AI 支援レール、または詳細レイアウトのレビュー領域に、整文候補と要約が表示されます。
...
標準レイアウト下部のスリム再生バーでは、音声ファイル名、再生位置、前後セグメントへの移動、再生・停止、波形シーク、音量を操作できます。詳細レイアウトでは、従来相当の高さを持つ再生バーで同じ操作ができます。最下部のステータスバーには、保存先、初回チェック、空き容量、メモリ、CPU、GPU の状態が表示されます。
